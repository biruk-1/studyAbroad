...
const Recommendations = () => {
  ...
  return (
    <div className="py-16 bg-gray-dim font-open-sans">
      <div className="container">
        <h2 className="text-3xl font-bold text-center mb-8 text-yellow-400">Find Your Match</h2>
        <form onSubmit={fetchRecommendations} className="max-w-lg mx-auto bg-gray-dim p-8 rounded-xl shadow-custom space-y-6">
          <div>
            <label htmlFor="budgetRange" className="block mb-1 font-semibold text-yellow-400">Budget</label>
            <select
              id="budgetRange"
              value={budgetRange}
              onChange={(e) => setBudgetRange(e.target.value)}
              className="w-full border p-3 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-400 bg-gray-700 text-white"
            >
              <option value="">Any Budget</option>
              <option value="$10K-$20K">$10K - $20K</option>
              <option value="$20K-$30K">$20K - $30K</option>
          ...
          <div>
            <label htmlFor="fieldOfStudy" className="block mb-1 font-semibold text-yellow-400">Field</label>
            <select
              id="fieldOfStudy"
              value={fieldOfStudy}
              onChange={(e) => setFieldOfStudy(e.target.value)}
          ...
          <div>
            <label htmlFor="preferredCountry" className="block mb-1 font-semibold text-yellow-400">Country</label>
            <select
              id="preferredCountry"
              value={preferredCountry}
              onChange={(e) => setPreferredCountry(e.target.value)}
          ...
          <div className="flex gap-4">
            <button
              type="submit"
              className="flex-1 bg-yellow-400 text-black py-3 px-4 rounded-xl hover:bg-yellow-600 transition-all duration-300"
              disabled={isLoading}
            >
              {isLoading ? "Finding..." : "Get Recommendations"}
            </button>
        ...
        {recommendations.length > 0 && (
          <div className="mt-12">
            <h3 className="text-2xl font-semibold text-center mb-6 text-yellow-400">Your Recommendations</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {recommendations.map((country) => (
                <div key={country.id} className="bg-gray-700 p-6 rounded-xl shadow-custom hover:shadow-lg transition-all duration-300">
                  <span className="text-5xl block text-center mb-4">{country.flag}</span>
                  <h4 className="text-xl font-semibold text-yellow-400 text-center">{country.name}</h4>
                  <p className="text-white text-center mt-2">{country.details}</p>
                  <p className="text-sm text-gray-300 mt-2"><strong>Cost:</strong> {country.cost_of_living}</p>
                  <p className="text-sm text-gray-300"><strong>Visa:</strong> {country.visa_process}</p>
                  <div className="mt-4 flex gap-4 justify-center">
                    <Link
                      to={`/countries/${country.name.toLowerCase()}`}
                    ...
                    <button
                      onClick={() => saveRecommendation(country.id)}
                      className="text-green-bright hover:underline flex items-center gap-1"
                    >
                      <FaSave /> Save
                    </button>
                  </div>
